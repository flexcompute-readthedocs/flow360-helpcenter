...
*The Symmetry boundary condition enforces flow symmetry across a plane, allowing you to simulate only a portion of the domain when the geometry and flow conditions are symmetric about that plane.*
...
- Multiple symmetry planes can be used when appropriate (e.g., quarter model)
- Each symmetry plane typically reduces computational domain by factor of 2
- Common combinations:
  - Vertical symmetry plane (half model)
  - Vertical + horizontal planes (quarter model)
  - Multiple planes for sectional models of axisymmetric geometries
