...
- **Example:** `[Box.from_principal_axes(name="porous_zone", axes=[(0, 1, 0), (0, 0, 1)], center=(0, 0, 0), size=(0.2, 0.3, 2))]`
...
### Region Definition
- **Volume Selection**: The region where the porous medium model applies
- **Coordinate System**: Local or global orientation of the porous medium
  - The axes of the entity must be specified to serve as the principal axes of the porous medium material model

### Resistance Properties
- **Viscous Resistance Coefficients (Darcy)**: Linear pressure drop coefficients (proportional to velocity)
  - Specified as a 3D vector, with one value for each principal direction
- **Inertial Resistance Coefficients (Forchheimer)**: Quadratic pressure drop coefficients (proportional to velocity squared)
  - Specified as a 3D vector, with one value for each principal direction
- **Directional Settings**: Different coefficients for x, y, z directions (for anisotropic media)

### Thermal Settings
- **Volumetric Heat Source**: Optional heat generation within the porous volume
  - Can be specified as a constant value or as a function of position/time

## Resistance Models

### Darcy's Law
For low Reynolds number flows, the pressure drop is linearly proportional to velocity:
- Δp = -μ/α v
- Where μ is dynamic viscosity, α is permeability, and v is velocity

### Forchheimer Extension
For higher Reynolds number flows, adds a quadratic term:
- Δp = -μ/α v - C ρ|v|v
- Where C is the inertial resistance coefficient and ρ is density
...
2. Plot Δp/L vs. velocity
3. Fit to quadratic equation: Δp/L = av + bv²
4. Viscous resistance = a/μ
5. Inertial resistance = b/ρ

### From Known Loss Coefficients
1. For screens/perforated plates with known loss coefficient K
2. Inertial resistance = K/(2Δx)
3. Where Δx is the medium thickness
