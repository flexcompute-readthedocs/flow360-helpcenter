...
| Velocity    | Dimensional velocity ($U_{dim}$)            | primitiveVars in all outputs                |
| Pressure    | $ρ_∞ × U_{dim}^2$                           | primitiveVars, nodeForcesPerUnitArea        |
| Temperature | Freestream Temperature ($T_∞$)                 | T in volume outputs                         |
| Heat Flux   | $ρ_∞ × U_{dim}^3$                           | heatFlux in surface outputs                 |
| Force (BET/AD/Porous Media) | $ρ_∞ × U_{dim}^2× (Grid Unit Length)^2$ | Forces in BET/AD/Porous Media outputs       |
| Moment (BET/AD/Porous Media) | $ρ_∞ × U_{dim}^2× (Grid Unit Length)^3$ | Moments in BET/AD/Porous Media outputs      |

### **Dimensional Velocity ($U_{dim}$)**

The dimensional velocity ($U_{dim}$) is the reference velocity used for nondimensionalizing velocity-related variables (velocity fields, heat flux, angular speeds, etc.):

- For `AerospaceCondition`: $U_{dim} = C_∞$ (speed of sound)
- For `LiquidOperatingCondition`: $U_{dim}$ is the reference velocity (if set) or velocity magnitude (if reference velocity is not set)

> **Note**: It is important to distinguish $U_{dim}$ from $U_{ref}$ (reference velocity used for force and moment coefficients). While both may have the same value in some cases, they serve different purposes:
> - $U_{dim}$ is used for nondimensionalizing velocity-related variables (velocity fields, heat flux, volumetric heat sources, angular speeds, etc.)
...
>
> The reference velocity for coefficients ($U_{ref}$) can be set via the `Mach` or `MachRef` parameters in the operating condition and may differ from the dimensional velocity ($U_{dim}$) used for other nondimensionalization.
