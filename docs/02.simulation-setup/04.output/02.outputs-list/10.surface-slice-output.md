...
- **Example:** `"wing_slices"`
- **Notes:** 
  - Choose a descriptive name that helps identify the purpose of this output.
...
- **Example:** 
  ...
  ```
- **Notes:** 
  - Each slice requires a name, origin point (3D coordinates), and normal vector (does not need to be normalized).
...
- **Notes:** 
  - Must reference boundary names from your mesh.
...
- **Notes:** 
  - See the Available Output Fields section below for a complete list.
...
- **Default:** `"paraview"`
- **Notes:** 
  - Currently only "paraview" format is supported.
