...
2. Plot $\frac{\Delta p}{L}$ vs velocity
3. Fit to quadratic equation: $\frac{\Delta p}{L} = av + bv^2$ 
4. Viscous resistance = $\frac{a}{\mu}$
5. Inertial resistance = $\frac{b}{\rho}$

- From Known Loss Coefficients
1. For screens/perforated plates with known loss coefficient $K$
2. Inertial resistance = $\frac{K}{2 \Delta x}$
3. Where $\Delta x$ is the medium thickness

- From Correlations
  For common geometries like packed beds:
  - Ergun equation: $\frac{\Delta p}{L} = 150 \mu \frac{(1 - \varepsilon)^2 v}{\varepsilon^3 d^2} + 1.75 \rho \frac{(1 - \varepsilon) v^2}{\varepsilon^3 d}$
  - Where $\varepsilon$ is porosity and $d$ is particle diameter
...
<details>
<summary><h3 style="display:inline-block"> 🔒 Limitations</h3></summary>
