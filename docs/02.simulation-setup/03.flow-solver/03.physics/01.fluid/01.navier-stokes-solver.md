...
| **Linear solver - max iterations** | Linear solver maximum iterations |
...
*Controls how often the Navier–Stokes equations are evaluated within each time step.* 

- **Default:** `1` (evaluate at each step)
- **Example:** `2`–`4` (reduced frequency for well-behaved cases)
>**Notes:** 
>- `1` provides the strongest coupling and most robust convergence
>- Higher values can reduce cost but may slow nonlinear convergence
>- If increased, monitor residuals and integral forces; revert to `1` if oscillations grow

### **Linear solver***

*Configures the iterative solver solved at each pseudo-step for Navier-Stokes equations.*

- **Default:** `max_iterations = 30`
- **Example:** `max_iterations = 50`
>**Notes:** 
>- Increasing `max_iterations` can improve robustness at added cost per pseudo-step
...
- **Example:** `10`
>**Notes:**
>- Useful for stiff cases or rapidly changing flows
