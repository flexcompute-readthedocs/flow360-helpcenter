...
>**Notes:** 
>- Non-dimensional values (normalized by dimensional velocity $U_{dim}$: speed of sound for `AerospaceCondition`, or reference velocity/velocity magnitude for `LiquidOperatingCondition`)
...
- **How are initial conditions normalized in Flow360?**  
  > Initial condition values are specified in non-dimensional form. Density is normalized by freestream density, velocities by the dimensional velocity ($U_{dim}$) which is the speed of sound for `AerospaceCondition`, or the reference velocity (if set) or velocity magnitude (if not) for `LiquidOperatingCondition`. Pressure is normalized by freestream dynamic pressure. The actual dimensional values depend on the operating condition settings. Note that fluid properties such as viscosity and thermal conductivity are taken from the operating condition settings rather than the initial condition.
