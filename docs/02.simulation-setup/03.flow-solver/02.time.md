...
| **Advanced** |
| `CFL Type` | Adaptive or Ramp CFL setting | always |
| `Min (CFL)` | Mininal allowable CFL | `CFL type` is `Adaptive CFL` |
...
| `Convergence limiting factor` | Parameter controlling the agressiveness of the adaptation algorithm | `CFL type` is `Adaptive CFL` |
| `Initial (CFL)` | The initial CFL value | `CFL type` is `Ramp CFL` |
| `Final (CFL)` | The final CFL value | `CFL type` is `Ramp CFL` |
| `Ramp steps` | the amount of time steps that the CFL will increase | `CFL type` is `Ramp CFL` |
...
| **Advanced** |
| `Maximum Pseudo Steps` | Maximum iterations within each physical step (unsteady) | always |
| `CFL Type` | Adaptive or Ramp CFL setting | always |
| `Min (CFL)` | Mininal allowable CFL | `CFL type` is `Adaptive CFL` |
| `Max (CFL)` | Maximum allowable CFL | `CFL type` is `Adaptive CFL` |
| `Max relative change` | Parameter responsible for the CFL increase | `CFL type` is `Adaptive CFL` |
...
| `Final (CFL)` | The final CFL value | `CFL type` is `Ramp CFL` |
| `Ramp steps` | the amount of time steps that the CFL will increase | `CFL type` is `Ramp CFL` |
| `Order of Accuracy` | Temporal accuracy (unsteady) | always |
