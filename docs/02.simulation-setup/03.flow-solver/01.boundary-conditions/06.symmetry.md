...
| **Option** | **Description** |
|------------|-----------------|
| `Assigned boundaries` | Geometric boundaries to apply the symmetry condition |
...
- **Notes:** Assign the boundaries by selecting from the list using the + button or select graphically in the viewer region. Can reference surfaces by name or pattern. Compatible with the `AutomatedFarfield` feature that creates appropriate symmetry plane boundaries.
