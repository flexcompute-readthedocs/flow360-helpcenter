...
| **Velocity direction** | Primary direction of the incoming flow | optional |
| **Total pressure** | Stagnation pressure at the inlet | subsonic flows |
| **Mass flow rate** | Mass flow through the inlet | specify a mass flow rate for subsonic flows |
| **Supersonic** | Supersonic flow specification | supersonic flows |
...
- **Required:** Optional (if not specified, the boundary patch normal is used)
- **Options:** Specify velocity axis
- **Example:** [1,0,0]
>**Note:** This specifies a unit vector indicating the direction of the incoming flow. If not specified, the boundary patch normal is used to determine the flow direction.

### **Total pressure**

*The stagnation (total) pressure of the flow at the inlet boundary. Used for both subsonic flows.*
...
*The mass flow rate through the inlet boundary. The solver adjusts the boundary total pressure to achieve the specified flow rate.*

- **Additional variables:**
  - **Value**: Required. Specify desired mass flow rate and units.
  - **Ramp steps**: Optional. Number of pseudo time steps over which to gradually increase the mass flow rate to the specified value. Default is `0` (applied immediately). Example: `100` steps. This helps with simulation stability for high flow rates.
>**Note:** The solver adjusts the inlet velocity profile to achieve the specified mass flow rate while maintaining the specified total temperature and computed static pressure distribution.

### **Supersonic**

*Specifies the inflow using both total and static pressure for supersonic flows. This fully determines the flow state at the inlet.*

- **Additional variables:**
  - **Total pressure:** Required. Total pressure at the inlet.
  - **Static pressure:** Required. Static pressure at the inlet.
>**Note:** For supersonic flows, both total and static pressure are directly specified, fully determining the flow state at the inlet. This is necessary because in supersonic flows, information cannot propagate upstream, so the complete flow state must be prescribed at the boundary. Unlike subsonic flows where the static pressure is determined iteratively, supersonic flows require all flow properties to be specified upfront.

>**Tip:** It is useful to use the isentropic relations to specify the velocity when setting up supersonic inflow conditions. The isentropic relations allow you to relate the total and static pressures to the Mach number, which can then be used to determine the velocity based on the total temperature and the speed of sound.
