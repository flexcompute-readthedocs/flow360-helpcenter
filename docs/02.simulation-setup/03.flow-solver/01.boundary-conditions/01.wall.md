...
| `Assigned boundaries` | Geometric boundaries to apply the wall condition | always |
...
- **Notes:** Fixed walls have zero velocity, while other types require additional specifications.
...
Static Pressure for a slater porous bleed model.
...
Porosity for a slater porous bleed model.

- **Required when `Slater porous bleed` selected.**
- **Example:** 0.3
- **Notes:** can be between 0 and 1.

#### `Activation step`

Pseudo step in the simulation at which the model is activated .
