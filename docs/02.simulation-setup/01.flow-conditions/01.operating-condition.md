...
*Operating condition form allows to specify the properties of the fluid that is simulated as well as its velocity in relation to the analyzed object. The values defined are also used for the calculations of force coefficients. The form allows to choose between two types of a fluid: gas and liquid.*

---
## 📋 **Available Options**

### Gas
The simulated fluid is consiered an ideal gas.
...
| `Temperature offset` | Temperature difference from the standart atmosphere at a given altitude | `Thermal state` is `Standard atmosphere` | 
...
An offset from the temperature taken from the standard atmosphere.
 ...
 - **Default:** 0 K
 - **Example:** -10 K
 - **Notes:** If the altitude is set to 10000 m, the temperature is 223.3 K according to the US standard atmosphere. With the temperature offset of -10 K, the effective freestream temperature will be 213.3 K.
