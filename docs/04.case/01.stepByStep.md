...
### Mesh Unit
The Mesh Unit is the length unit used in the mesh file. It can be m (meter), cm (centimeter), mm (millimeter), inch, or feet. Used to calculate $L_{gridUnit}$.

### Reference area
  
The Reference Area, `refArea`, is used to non-dimensionalize lift coefficient (CL), drag coefficient (CD), force, and moment coefficients. 
  
For example:
$CL = Lift / (1/2 * \rho_{inf} * U_{ref}^2 * (refArea * L_{gridUnit}^2))$
$CD = Drag / (1/2 * \rho_{inf} * U_{ref}^2 * (refArea * L_{gridUnit}^2))$
$CFx = (X Force) / (1/2 * \rho_{inf} * U_{ref}^2 * (refArea * L_{gridUnit}^2))$
$CFy = (Y Force) / (1/2 * \rho_{inf} * U_{ref}^2 * (refArea * L_{gridUnit}^2))$
$CFz = (Z Force) / (1/2 * \rho_{inf} * U_{ref}^2 * (refArea * L_{gridUnit}^2))$

Here, $\rho_{inf}$ is the freestream density and $U_{ref}$ is the reference velocity calculated by $U_{ref} = MachRef * C_{inf}$,
where $C_{inf}$ is the freestream speed of sound, and by default, the reference Mach number (`MachRef`) is set to be equal to the freestream 
Mach number (`Mach`). However, when `Mach==0`, `MachRef` needs to be specified explicitly. 
In this case, it is equivalent to explicitly set a non-zero reference velocity in the next section for freestream parameters.
...
For example:
1. A z-directional force acting at an x-coordinate greater than the x-coordinate of the Moment Center would generate a negative y-moment.
2. An x-directional force acting at an z-coordinate greater than the z-coordinate of the Moment Center would generate a positive y-moment.
3. An x-directional force acting at an y-coordinate greater than the y-coordinate of the Moment Center would generate a negative z-moment.
4. A y-directional force acting at an x-coordinate greater than the x-coordinate of the Moment Center would generate a positive z-moment.
5. A y-directional force acting at an z-coordinate greater than the z-coordinate of the Moment Center would generate a negative x-moment.
6. A z-directional force acting at an y-coordinate greater than the y-coordinate of the Moment Center would generate a positive x-moment.

### Moment Length

These moment reference lengths are used to non-dimensionalize moment coefficients, according to the definitions below. 
Their values as entered here are non-dimensional with respect to the physical length scale ($L_{gridUnit}$) represented by length unit used in the mesh file.
For example:
1. $Mx = Mx / (1/2 * \rho_{inf} * U_{ref}^2 * (refArea * L_{gridUnit}^2) * (momentLength_x * L_{gridUnit}))$
2. $CMy = My / (1/2 * \rho_{inf} * U_{ref}^2 * (refArea * L_{gridUnit}^2) * (momentLength_y * L_{gridUnit}))$
3. $CMz = Mz / (1/2 * \rho_{inf} * U_{ref}^2 * (refArea * L_{gridUnit}^2) * (momentLength_z * L_{gridUnit}))$ 

Here, $(Mx, My, Mz)$ are the moment components with respect to the Moment Center, $\rho_{inf}$ is the freestream density.
Notably, $U_{ref}$ is the reference velocity calculated by $U_{ref} = MachRef * C_{inf}$, where $C_{inf}$ is the freestream speed of sound.
...
- At the standard sea level condition, enter *1.225*. Be sure to use the value with a unit of kg/m^3.
...
- The freestream velocity, $U_{inf}$, is calculated by $U_{inf} = Mach * C_{inf}$,
where Mach is the freestream Mach number and $C_{inf}$ is the freestream speed of sound.
...
By default, the freestream velocity, $U_{inf}$, is set to be the reference velocity.
...
- `Mach` $= U_{inf} / C_{inf}$
- `MachRef` $= U_{ref} / C_{inf}$

Where, $C_{inf}$ is the freestream speed of sound.
Also, when your freestream velocity is zero, i.e. $U_{inf}==0$ m/s, you must enter a finite value for your reference velocity.
...
The velocity components, both at boundaries and inside domain, are initialized according to the following equations, based on the angle of attack, $\alpha$ and the side slip angle, $\beta$:

$U_X =  (freestreamVelocity / C_{inf}) * \cos(\beta) * \cos(\alpha)$
$U_Y = -(freestreamVelocity / C_{inf}) * \sin(\beta)$
$U_Z =  (freestreamVelocity / C_{inf}) * \cos(\beta) * \sin(\alpha)$

Note that the velocity components on the left hand side are non-dimensional as used in Flow360 solver. $C_{inf}$ is the freestream speed of sound. Also, for flow simulations with a freestream boundary condition, flow fields will be initialized according to this boundary condition.
...
Steady simulation attempts to converge to a steady-state flow solution. Answer no for an unsteady simulation that solves for unsteady flow physics. For the latter, you will need to enter a physical time step size and the total number of time steps.

### Time step size for unsteady simulation (seconds)
  
A second-order implicit time stepping scheme will be used to converge each time step. Enter the physical time step size here. Note that this time step size will be nondimensionalized using the freestream speed of sound and the mesh length unit when generating the solver input JSON file.

### How many time steps do you want to simulate?

A second-order implicit time stepping scheme will be used to converge each time step. Enter number of time steps here. Note that a nonlinear iteration will be used to converge each time step. You will need to enter the number of nonlinear iterations in the next field.
  
### The maximum number of nonlinear iterations used to converge each physical time step
  
A nonlinear iteration (quasi-Newton) will be used to converge each physical time step. When the maximum number of iterations is reached, the solver will proceed to the next physical time step, even if the final nonlinear residuals are still higher than the tolerance.
...
For steady simulations, the physical time step size is equivalent to "inf", and only one physical time step is needed. The settings for the CFL number is the same as above.
...
The CFL number will ramp up from the initial to the final value during the first part of the iterations. The ramping steps control how long the first part is. During the rest of the iterations, the CFL number will stay at the final value. See [initial CFL number](/04.case/01.stepByStep.html#initial-cfl-number) for more details.
...
As a rule of thumb, to visualize vortical structures in a flow field for an aeronautical application, plot iso-surfaces of a finite small positive threshold on Q values. For example, a threshold roughly equals to (Freestream Mach Number / Reference Length)^2.
...
ISSUE #051: The description is rewritten as the below. (Notably, as explained earlier, this expression on skin friction is not exactly correct, at least to me. Skin friction by its name refers to wall shear stresses. Wall normal stress has nothing to do with this. Equivalently, as output from Flow360, CfTangent is the wall shear stress commonly termed as skin friction coefficient. For a moderate flow Mach number, CfNormal is usually very small, this is why the differences between Cf and CfTangent are barely noted, as for most of aeronautical applications. But the term skin friction coefficient by definition is referred to CfTangent. Please verify this.)

  This is the magnitude of the non-dimensionalized viscous stress vector exerted by a wall on the fluid, with both wall normal and wall tangential components, i.e. Cf = sqrt(CfNormal^2 + CfTangent^2). 
  Note that CfTangent as also output from Flow360 is the quantity commonly termed as skin friction coefficient.
  
### Cf Vector

  Skin friction coefficient vector
  
ISSUE #052: The description is rewritten as the below.

  This is the directional vector of the non-dimensionalized viscous stress vector exerted by a wall on the fluid, with three components as of Cfx, Cfy and Cfz.