...
| `Curvature resolution angle` | Maximum angle, a single surface mesh element can span | degree |
| `Boundary layer growth rate` | Growth rate for prismatic boundary layer elements | dimensionless |
| `Boundary layer first layer thickness` | First layer thickness for volumetric anisotropic layers | length (e.g., mm) |
...
*Controls the size progression of mesh elements grown from edges, affecting the transition between regions of different refinement.*

- **Default:** 1.2
- **Units:** Dimensionless
- **Notes:** 
  - Values closer to 1.0 produce more gradual transitions
